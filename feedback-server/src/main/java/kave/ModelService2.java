...
public class ModelService2 {
	...
	// it!
	/*
	 * @Inject public ModelService2(File dataFolder, File tmpFolder, IoUtils io,
	 * UniqueFileCreator tmpufc, UniqueFileCreator dataUfc) throws IOException {
	 * this.dataFolder = dataFolder; this.dataUfc = dataUfc; this.tmpufc =
	 * tmpufc; this.tmpFolder = tmpFolder; this.io = io; enforceFolders(); }
	 */
	@Inject
	public ModelService2(File dataFolder, File tmpFolder, UploadCleanser cleanser, UniqueFileCreator tmpufc,
	...
	public Response receiveJSON(JsonNode json) throws JSONException, IOException {
		...
		String response = convertFile(file_content, fileName);
	...
	private String convertFile(String file_string, String file_name) throws IOException {
		byte[] bytes = Base64.decode(file_string);
		String name = new SimpleDateFormat("HH-mm-ss").format(new Date());
		String fileName = file_name + "-" + name + ".zip";
		...
		fop.flush();
		fop.close();
		return fileName + "uploaded to Server @ " + dataFolder.getPath();
